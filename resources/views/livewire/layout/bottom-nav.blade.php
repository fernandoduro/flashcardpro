...
<nav class="sm:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 z-50">
    <div class="flex justify-around h-16">
        ...
        <a href="{{ route('decks.index') }}"
           class="flex flex-col items-center justify-center w-full text-center px-2 {{ request()->routeIs('decks.index') ? 'text-primary-600' : 'text-gray-500' }} hover:bg-gray-50 transition-colors">
        ...
        <a href="{{ route('statistics.index') }}"
           class="flex flex-col items-center justify-center w-full text-center px-2 {{ request()->routeIs('statistics.index') ? 'text-primary-600' : 'text-gray-500' }} hover:bg-gray-50 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
